...
const propertyName = "BezelMiami";
class bezelMiamiTracker extends trackerInterface {
  constructor() {
    super();
  }
  ...
  async trackPrices() {
    ...
    try {
      ...
      const floorPlansData = await axiosInstance.get(
        "https://www.bezelmiami.com/ajax/planspricing/"
      );
      ...
      const responseData = await axiosInstance.get(
        "https://www.bezelmiami.com/ajax/availability/"
      );
      ...
      for (const currentAptWithPrice of responseData.data) {
        if (!currentAptWithPrice.FloorPlanID) continue;
...
export default bezelMiamiTracker;
